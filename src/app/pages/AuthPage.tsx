    ...
    <div className="min-h-screen bg-background flex items-center justify-center relative overflow-hidden px-4 py-12"
      ...
      <div className="relative z-10 w-full max-w-md border border-border overflow-hidden" style={{ background: "#141418" }}>
        ...
        <div className="px-8 mb-6">
          <div className="grid grid-cols-2 border border-border">
            {(["signin", "signup"] as const).map(t => (
              <button key={t} onClick={() => { setTab(t); setError(""); setMessage(""); }} className="py-2.5 text-xs transition-all duration-200"
        ...
        <form onSubmit={handleSubmit} className="px-8 pb-8 space-y-4">
          ...
          {error && (
            <div className="text-xs text-red-500 bg-red-500/10 border border-red-500/20 p-3" style={{ fontFamily: BODY, borderRadius: 0 }}>
              {error}
            </div>
          )}
          {message && (
            <div className="text-xs text-green-500 bg-green-500/10 border border-green-500/20 p-3" style={{ fontFamily: BODY, borderRadius: 0 }}>
              {message}
            </div>
          )}
